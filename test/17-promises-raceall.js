...
test('Use `Promise.all` to wait for many promises to resolve', t => {
  ...
  return Promise.all([delay(200).then(() => 'foo'),
                      delay(500).then(() => 'bar'),
                      delay(300).then(() => 'baz'),
    ...
    .then(values => {
      ...
      t.deepEqual(values, ['foo', 'bar', 'baz', 'qux']); // <-- What is `values`?

      // Let's prove that the promises executed in parallel:

      t.true(duration >= 500); // <-- Took at least as long as the longest delay.
      t.true(duration < 1000); // <-- But took less than the sum of all delays.
    })
});

test('`Promise.race` only waits for one promise to resolve', t => {
  t.plan(2);
  ...
  let r1 = Promise.race([delay(50, 'foo'), delay(10, 'bar')])
    .then(winner => t.is(winner, 'bar')); // <-- Fill in the blank

  let r2 = Promise.race(['foo', delay(10, 'bar')])
    .then(winner => t.is(winner, 'foo')); // <-- Fill in the blank

  // Our test framework actually waits for promises to resolve. Neat!
  return Promise.all([r1, r2]);
});

test('Exercise: Use `delay` and `Promise.race`to implement timeouts', t => {
  ...
  return Promise.race([
      delay(ms, Promise.reject('Timed out')),
      p.then((x) => x)
  ])
  }

  let p1 = getWithTimeout(delay(100, 'Data'), 25)
    .then(() => t.fail())  // <-- This must never execute
    .catch(reason => t.is(reason, 'Timed out'));

  let p2 = getWithTimeout(delay(100, 'More data'), 300)
